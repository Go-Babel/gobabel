...
void main() {
  group('addBabelInitializationToMainUsecase', () {
    ...
    test(
      'Should add WidgetsFlutterBinding.ensureInitialized() and Babel.instance.initialize() for Flutter project without binding',
      () async {
...
''');
        ...
        final bindingIndex = content.indexOf(
    ...
    test(
      'Should add Babel.instance.initialize() after existing WidgetsFlutterBinding.ensureInitialized()',
      () async {
        final mainFile = File('${tempDir.path}/lib/main.dart');
        await mainFile.writeAsString('''
import 'package:flutter/material.dart';
...
''');
    ...
    test(
      'Should only add Babel.instance.initialize() for Dart project',
      () async {
...
''');
    ...
    test(
      'Should not duplicate Babel initialization if already present',
      () async {
        final mainFile = File('${tempDir.path}/lib/main.dart');
        await mainFile.writeAsString('''
import 'package:flutter/material.dart';
...
''');
        ...
        final babelCount =
            'Babel.instance.initialize'.allMatches(content).length;
        expect(babelCount, 1);
      },
    );

    test(
      'Should add Flutter import when adding WidgetsFlutterBinding',
      () async {
        final mainFile = File('${tempDir.path}/lib/main.dart');
        await mainFile.writeAsString('''
void main() {
